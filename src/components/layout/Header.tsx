...
/* ------------------------------------------------------------------ */
/*  Mobile bottom-bar items (5 tabs max for thumb reach)               */
/* ------------------------------------------------------------------ */
const MOBILE_TABS = [
  {
    key: 'home' as const,
  ...
  },
  {
    ...
    icon: (active: boolean) => (
      <svg className="w-5 h-5" fill={active ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={active ? 0 : 1.8} d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" />
      </svg>
    ),
  },
  {
...
/* ------------------------------------------------------------------ */
/*  Language switcher (desktop top bar)                                 */
/* ------------------------------------------------------------------ */
      ...
      >
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex justify-between items-center h-14 md:h-20">
            ...
            {/* Mobile: Language + WhatsApp mini buttons */}
            <div className="flex md:hidden items-center gap-2">
                ...
                {(['en', 'id'] as Language[]).map((lang) => {
                  ...
                  return (
                    <button
                      key={lang}
                      onClick={() => setLanguage(lang)}
                      className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase transition-all duration-200 ${
                        language === lang
                          ? 'bg-brand-blue-800 text-white shadow-sm'
                          : 'text-gray-400'
                      }`}
                    >
                      <Flag className="w-4 h-3 rounded-sm overflow-hidden flex-shrink-0" />
                      {LANG_LABEL[lang]}
                    </button>
                  );
                })}
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="w-9 h-9 rounded-xl bg-whatsapp text-white flex items-center justify-center shadow-md"
                aria-label="WhatsApp"
              >
          ...
          {MOBILE_TABS.map((tab) => {
            ...
            return (
              <Link
                key={tab.href}
                href={tab.href}
                className={`flex flex-col items-center gap-0.5 min-w-[3.5rem] py-1 rounded-xl transition-all duration-200 ${
                  active
                    ? 'text-brand-blue-800'
              ...
              >
                ...
                <span className={`text-[10px] font-semibold leading-none ${active ? 'text-brand-blue-800' : 'text-gray-400'}`}>
                  {t.nav[tab.key]}
                </span>
              </Link>
            );

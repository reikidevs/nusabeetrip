...
Metadata is built in one place via `buildMetadata()` in `src/lib/seo.ts`. Every page sets its own canonical URL, hreflang variants, OpenGraph image, and JSON-LD. Collection pages emit `ItemList`; tour and rental detail pages emit `Service` with `Offer`. Rating markup is added only when genuine reviews are tied to a specific service and displayed on that page. Sitemap and robots are generated by `src/app/sitemap.ts` and `src/app/robots.ts`.
